...
export default function VerificationPage() {
  ...
  const getVerificationDisplay = (verification: Verification | undefined) => {
    if (!verification) {
      return { text: "No documents submitted", style: "bg-gray-100 text-gray-800" };
    }
    ...
    if (
      ...
      (verification.verificationStatus === "pending" || verification.verificationStatus === "in_review")
    ) {
      ...
      <main className="flex-1 w-11/12 max-w-5xl mx-auto py-10 space-y-10">
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          ...
          <div className="p-6 rounded-2xl shadow-md border bg-white hover:shadow-lg flex flex-col">
            ...
            <Button className="bg-[#E05265] text-white rounded-xl" onClick={() => setSelfieOpen(true)}>
              Take Selfie
            </Button>
            {profile?.verification?.livePhotoUrl && (
              <div className="mt-2 flex flex-col items-start">
                <img
                  ...
                  alt="Selfie Preview"
                  className="w-32 h-32 object-cover rounded-full border-2 border-[#E05265]"
                />
          ...
          <div className="flex gap-2 mt-3 flex-wrap">
            <span
              className={`px-2 py-1 rounded-full text-sm ${
                profile?.verification?.idVerified ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"
              }`}
            >
              ID
            </span>
            <span
              className={`px-2 py-1 rounded-full text-sm ${
                profile?.verification?.photoVerified ? "bg-green-100 text-green-800" : "bg-gray-100 text-gray-600"
              }`}
            >
              Selfie
            </span>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 z-50">
          <div className="bg-white p-6 rounded-2xl shadow-lg flex flex-col items-center">
            ...
            <div className="flex gap-4 mt-6">
              ...
              <Button variant="outline" onClick={() => setSelfieOpen(false)} className="rounded-xl">
                Cancel
              </Button>
            </div>
          </div>
